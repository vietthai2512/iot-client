...
const AddContainedSVG: React.FC<Props> = ({ size = 'md', theme = 'light' }) => {
  const returnOpacityFromTheme = (theme: string) => {
    switch (theme) {
      case 'light': {
        return '0.2';
      }
  ...
  return (
    <>
      <svg {...returnSize(size)} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
          opacity={returnOpacityFromTheme(theme)}
...
export default AddContainedSVG;
